...
#
# Remove asterisks from Human symbols...
#
...
from MRK_Marker m1
where _Organism_key = 2 and symbol like '*%'
and not exists (select 1 from MRK_Marker m2
where m2._Organism_key = 2 and m2.symbol = substring(m1.symbol, 2, char_length(m1.symbol) - 2))
for read only
go
